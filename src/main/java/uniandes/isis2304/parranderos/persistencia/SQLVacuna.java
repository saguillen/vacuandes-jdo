...
class SQLVacuna 
{
	 ...
	 */
	public SQLVacuna (PersistenciaParranderos pp)
	{
		this.pp = pp;
	}
	 ...
	 */
	public long adicionarVacuna (PersistenceManager pm, long idVacuna, String fabricante, long lote) 
	{
        Query q = pm.newQuery(SQL, "INSERT INTO " + pp.darTablaVacuna() + "(id, fabricante, idlote) values (?, ?, ?)");
        q.setParameters(idVacuna, fabricante, lote);
	 ...
	 * @return EL número de tuplas eliminadas
	 */
	public long eliminarVacunaPorId (PersistenceManager pm, long idVac)
	{
        Query q = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaVacuna() + " WHERE id = ?");
        q.setParameters(idVac);
        return (long) q.executeUnique();
	}
