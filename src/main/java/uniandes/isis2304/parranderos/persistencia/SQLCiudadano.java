...
class SQLCiudadano 
{
	 ...
	 */
	public SQLCiudadano (PersistenciaParranderos pp)
	{
		this.pp = pp;
	}
	 ...
	 */
	public long adicionarCiudadano (PersistenceManager pm, long idCiud, long edad, String ocupacion, long enfermedad, String grupo, long etapa, String estado, long idUsuario2, long idPuntoV) 
	{
        Query q = pm.newQuery(SQL, "INSERT INTO " + pp.darTablaCiudadano() + "(id, edad, ocupacion, enfermedad, grupo, etapa, estado, idusuario, idpuntov) values (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        q.setParameters(idCiud,edad,ocupacion,enfermedad,grupo,etapa,estado,idUsuario2,idPuntoV);
	...
	{
        Query q = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaCiudadano() + " WHERE nombre = ?");
	 ...
	 * @return EL número de tuplas eliminadas
	 */
	public long eliminarBarPorId (PersistenceManager pm, long idCiud)
	{
        Query q = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaCiudadano() + " WHERE id = ?");
        q.setParameters(idCiud);
        return (long) q.executeUnique();
	}
	...
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM " + pp.darTablaCiudadano()+ " WHERE nombre = ?");
		q.setResultClass(Bar.class);
		q.setParameters(nombreBar);
	...
	{
        Query q = pm.newQuery(SQL, "UPDATE " + pp.darTablaCiudadano() + " SET cantsedes = cantsedes + 1 WHERE ciudad = ?");
